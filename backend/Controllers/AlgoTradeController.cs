...
namespace MarginCoinAPI.Controllers
{
    ...
    public class AlgoTradeController : ControllerBase
    {
        ...
        public string MonitorMarket()
        {
            ...
            // Start the initialization in the background - don't block the HTTP response
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    _logger.LogWarning("Market monitoring initialization complete");

                    // Notify frontend that initialization is complete
                    await _hub.Clients.All.SendAsync("marketInitialized", JsonSerializer.Serialize(new
                    {
                        status = "ready",
                        symbolCount = _tradingState.CandleMatrix.Count,
                        message = "Market monitoring is now active"
                    }));
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to initialize market monitoring");
                    await _hub.Clients.All.SendAsync("marketInitialized", JsonSerializer.Serialize(new
                    {
                        status = "error",
            ...
            return "Market monitoring initialization started in background";
        }
        ...
        public IActionResult GetCandleData(string symbol)
        {
            try
            {
                // Create a temporary list to hold the candle data
                var candleMatrix = new List<List<Candle>>();

                // Fetch candles from Binance
                _binanceService.GetCandles(symbol, ref candleMatrix);

                if (candleMatrix.Count == 0 || candleMatrix[0].Count == 0)
                {
                    return NotFound(new { error = $"No candle data found for {symbol}" });
                }

                // Transform candles to a simple format for the frontend
                var candles = candleMatrix[0].Select(c => new
                {
                    x = c.T,
                    open = c.o,
                    high = c.h,
            ...
            {
                _logger.LogError(ex, "Failed to fetch candle data for {Symbol}", symbol);
                return StatusCode(500, new { error = $"Failed to fetch candle data: {ex.Message}" });
            }
        }
        ...
        private async Task LoadFreshCandleDataIntoMemory()
        {
            ...
            // Notify frontend that candle loading is starting
            await _hub.Clients.All.SendAsync("candleLoadProgress", JsonSerializer.Serialize(new
            {
                status = "loading",
                loaded = 0,
                total = targetSymbols.Count,
                message = $"Loading candle data for {targetSymbols.Count} symbols..."
            }));
            ...
            foreach (var symbol in targetSymbols)
            {
                try
                {
                    ...
                    if (tempMatrix.Count > 0 && tempMatrix[0].Count >= 50)
                    {
                        ...
                        // Send progress update every 5 symbols to avoid flooding
                        if (successCount % 5 == 0 || successCount == targetSymbols.Count)
                        {
                            await _hub.Clients.All.SendAsync("candleLoadProgress", JsonSerializer.Serialize(new
                            {
                                status = "loading",
                                ...
                                failCount = failCount,
                                message = $"Loaded {successCount}/{targetSymbols.Count} symbols..."
                            }));
                        }
                    }
                    else
            ...
            await _hub.Clients.All.SendAsync("candleLoadProgress", JsonSerializer.Serialize(new
            {
                ...
                successCount = successCount,
                failCount = failCount,
                message = $"Candle loading complete: {successCount} succeeded, {failCount} failed"
            }));
        }
        ...
        // Helper class to return entry decision with OpenAI analysis
        private class EntryDecisionResult
        {
            public bool ShouldEnter { get; set; }
            public OpenAIAnalysisResult OpenAIAnalysis { get; set; }
        }
        ...
        private async Task ReviewSpotMarket(List<Candle> symbolCandleList, IOrderService orderService, RuntimeTradingSettings runtime)
        {
                ...
                if (activeOrder == null && activeOrderCount < runtime.MaxOpenTrades)
            {
                ...
                if (entryDecision.ShouldEnter)
                {
                    ...
                    // Extract OpenAI data if available
                    string openAISignal = entryDecision.OpenAIAnalysis?.Signal;
                    double openAIScore = entryDecision.OpenAIAnalysis?.TradingScore ?? 0;
                    double openAIConfidence = entryDecision.OpenAIAnalysis?.Confidence ?? 0;
                    string openAIRiskLevel = entryDecision.OpenAIAnalysis?.RiskLevel;
                    string openAIReasoning = entryDecision.OpenAIAnalysis?.Reasoning;
        ...
        private async Task<EntryDecisionResult> EnterLongPosition(MarketStream marketData, List<Candle> symbolCandles, RuntimeTradingSettings runtime)
        {
            OpenAIAnalysisResult openAIAnalysis = null;
            ...
            if (entryTrendScore < _config.MinTrendScoreForEntry)
            {
                ...
                return new EntryDecisionResult { ShouldEnter = false, OpenAIAnalysis = null };
            }
            ...
            // OpenAI Enhanced Signal Check (runtime configurable) - LOGGING ONLY, NOT BLOCKING
            if (runtime.EnableOpenAISignals)
            {
                try
                {
                    ...
                    if (currentCandle != null)
                    {
                        ...
                        if (aiAnalysis != null)
                        {
                            // Store the analysis to return
                            openAIAnalysis = aiAnalysis;

                            var aiLogPayload = new
                            {
                                symbol = marketData.s,
                                aiAnalysis.Signal,
                                aiAnalysis.TradingScore,
                            ...
                            _logger.LogInformation("OpenAI Analysis (logging only): {Payload}", JsonSerializer.Serialize(aiLogPayload));

                            // Check if OpenAI signal meets our criteria (logging only, not blocking)
                            var isStrongSignal = OpenAIPredictionService.IsStrongSignal(aiAnalysis,
                            ...
                            if (!isStrongSignal)
                            {
                                _logger.LogInformation(
                                    "OpenAI would have rejected {Symbol}: Signal not strong enough. " +
                                    "Signal={Signal}, Score={Score}, Confidence={Confidence:P}, Need: Score>={MinScore}, Confidence>={MinConf:P} (NOT BLOCKING - logging only)",
                            ...
                            {
                                _logger.LogInformation("OpenAI would have approved {Symbol}: {Payload}", marketData.s, JsonSerializer.Serialize(aiLogPayload));
                            }
                        }
                        else
                        {
            ...
            if (_tradingState.OnHold.TryGetValue(marketData.s, out var isOnHold) && isOnHold)
            {
                ...
                return new EntryDecisionResult { ShouldEnter = false, OpenAIAnalysis = openAIAnalysis };
            }
            ...
            return new EntryDecisionResult { ShouldEnter = true, OpenAIAnalysis = openAIAnalysis };
        }
        ...
        private async Task ReviewOpenTrade(string symbol, IOrderService orderService, ApplicationDbContext dbContext, RuntimeTradingSettings runtime)
        {
            ...
            lock (candleMatrixLock)
            {
                ...
                // Skip risk checks for orders that are not filled yet (prevents immediate stop loss on pending entries)
