...
namespace MarginCoinAPI.Controllers
{
    ...
    public class BinanceController : ControllerBase
    {
        ...
        public async Task<BinanceAccount> BinanceAccount()
        {
            ...
            // Ensure SymbolBaseList is populated
            if (_tradingState.SymbolBaseList.Count == 0)
            {
                _tradingState.SymbolBaseList = _symbolService.GetTopSymbols(_tradingConfig.NumberOfSymbols);
            }

            var symbolsWeTrade = _tradingState.SymbolBaseList;
            ...
            myAccount.balances = myAccount.balances
                ...
                .ThenBy(b => symbolsWeTrade.FirstOrDefault(s => s.SymbolName.Replace("USDC", "").Replace("USDT", "") == b.asset)?.Rank ?? int.MaxValue)
                .ThenBy(b => b.asset)
                .ToList();
