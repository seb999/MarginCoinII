...
namespace MarginCoinAPI.Service
{
        ...
        string openAISignal = null, double openAIScore = 0, double openAIConfidence = 0, string openAIRiskLevel = null, string openAIReasoning = null)
    {
        ...
        Order myOrder = new Order
        {
            ...
            OpenAIConfidence = openAIConfidence,
            OpenAIRiskLevel = openAIRiskLevel,
    ...
    public async Task SellLimit(Order dbOrder, MarketStream symbolSpot, string closeType, double exitAiScore = 0, string exitAiPrediction = "")
    {
        ...
        // Calculate intended quantity from database
        var intendedQty = dbOrder.QuantityBuy - dbOrder.QuantitySell;

        // Get actual available balance from Binance
        var actualBalance = GetAvailableAssetBalance(dbOrder.Symbol);

        // Use the MINIMUM of intended quantity and actual balance
        var qtyToSell = Math.Min(intendedQty, actualBalance);
        var qty = Math.Round(qtyToSell, nbrDecimalQty);

        if (qty <= 0)
        {
            _logger.LogWarning("Cannot sell {Symbol}. Intended: {Intended}, Actual balance: {Actual}. Rounded quantity: {Qty}",
                dbOrder.Symbol, intendedQty, actualBalance, qty);
            return;
        }

        if (actualBalance < intendedQty)
        {
            _logger.LogWarning("Adjusting sell quantity for {Symbol}. Database shows {DbQty} but only {ActualQty} available in portfolio. Selling {SellQty}",
                dbOrder.Symbol, intendedQty, actualBalance, qty);
        }
        ...
        if (myBinanceOrder == null)
        {
            _logger.LogWarning("SellLimit returned null for {Symbol}. Order may have failed on exchange.", dbOrder.Symbol);
            return;
        }
    ...
    public async Task SellMarket(Order dbOrder, string closeType, double exitAiScore = 0, string exitAiPrediction = "")
    {
        ...
        // Use the MINIMUM of intended quantity and actual balance
        // This prevents trying to sell more than we have
        var qtyToSell = Math.Min(intendedQty, actualBalance);

        if (qtyToSell <= 0)
        {
            _logger.LogWarning("Cannot sell {Symbol}. Intended: {Intended}, Actual balance: {Actual}. Quantity to sell: {Qty}",
                dbOrder.Symbol, intendedQty, actualBalance, qtyToSell);
            return;
        }
        ...
        BinanceOrder myBinanceOrder = _binanceService.SellMarket(dbOrder.Symbol, qtyToSell);
    ...
    private (int nbrDecimalPrice, int nbrDecimalQty) GetOrderParameters(MarketStream symbolSpot)
    {
        try
        {
            ...
            if (ticker == null || ticker.symbols == null || ticker.symbols.Count == 0)
            {
                _logger.LogError("Ticker returned null or empty for {Symbol}. Symbol may not exist on this exchange.", symbolSpot.s);
                // Return default values for USDC pairs
                return (5, 2); // Default: 5 decimals for price, 2 for quantity
            }

            if (ticker.symbols[0].filters == null || ticker.symbols[0].filters.Count < 2)
            {
                _logger.LogWarning("Ticker filters missing for {Symbol}. Using default values.", symbolSpot.s);
                return (5, 2);
            }
        ...
        {
            _logger.LogError(ex, "Error getting order parameters for {Symbol}. Using default values.", symbolSpot.s);
            return (5, 2); // Default: 5 decimals for price, 2 for quantity
        }
    }
    ...
    private double GetAvailableAssetBalance(string symbol)
    {
        try
        {
            // Extract asset from symbol (e.g., "BTCUSDC" -> "BTC")
            var asset = symbol.Replace("USDC", "").Replace("USDT", "");

            var account = _binanceService.Account();
            if (account?.balances == null)
            {
                _logger.LogWarning("Failed to retrieve account balance for {Asset} - account or balances is null", asset);
                return 0;
            }

            var assetBalance = account.balances.FirstOrDefault(b => b.asset == asset);
            if (assetBalance != null && double.TryParse(assetBalance.free, NumberStyles.Any, CultureInfo.InvariantCulture, out double balance))
            {
                return balance;
            }

            _logger.LogWarning("{Asset} balance not found in account", asset);
            return 0;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to retrieve asset balance for {Symbol}", symbol);
            return 0;
        }
    }
